class UsersController < ApplicationController
    ...
    def like
      ...
      @like_matches = Like.where(like_user_id: user.id).page(params[:page]).per(PER)
      @match = []
      @like_count = []
      @evaluate_user = []
      @like_matches.each do |match|
        @evaluate_user.push(User.where(id: match.post_user_id)[0])
        ...
        @match.push(like)
        @like_count.push(Like.where(post_user_id: like[:user_id], match_id: like[:match_id]).size)
      end
